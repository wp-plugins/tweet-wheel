...
require_once( 'class-tw-metaboxes-fields.php' );
require_once( 'metaboxes/tweet-text.php' );
...
class TW_Meta_Box {
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'cmb-scripts', TW_PLUGIN_URL . '/assets/js/cmb.js', array( 'jquery' ) );
	...
	function enqueue_styles() {
		...
		if ( version_compare( get_bloginfo( 'version' ), '3.8', '>=' ) )
			wp_enqueue_style( 'cmb-styles', TW_PLUGIN_URL . "/assets/css/dist/cmb$suffix.css" );
		else
			wp_enqueue_style( 'cmb-styles', TW_PLUGIN_URL . '/assets/css/legacy.css' );
...
function tw_metaboxes_init() {
	...
	if ( ! empty( $meta_boxes ) )
		foreach ( $meta_boxes as $meta_box )
			new TW_Meta_Box( $meta_box );

}
add_action( 'tweetwheel_init', 'tw_metaboxes_init', 50 );
